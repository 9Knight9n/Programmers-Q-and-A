...
class ChatroomInfo extends Component {
  ...
  updateJoinState = (joinState) => {
    if (this.state.isDiscussion) this.props.changeJoinState(joinState);
    this.setState({
      isJoined: joinState,
    });
  };
  ...
  hideEditor = (submit) => {
    ...
    if (submit) this.handleSubmitQuestion();
  };
  ...
  handleSubmitQuestion = async () => {
    ...
    if (this.props.loadQuestions) this.props.loadQuestions();
  };
  ...
  loadData = async () => {
    ...
    if (data) {
      this.setState({
        ...
        chatroomLink:
          "http://localhost:3000/" +
          (this.state.isDiscussion ? "discussion" : "qanda") +
          this.state.chatroomId,
      });
    }
  ...
  loadJoinState = async () => {
    ...
    let config = {
      ...
      needToken: false,
      type: "post",
      formKey: ["id", "chatroomId"],
      formValue: [sessionStorage.getItem("id"), this.props.Cid],
    };
    let data = [];
    // console.log("outside 0",data)
    data = await request(config);
        ...
        <div className="chatroomInfo-infoElements d-flex flex-row">
          <div className="chatroomInfo-avatar-name-contexet-link d-flex flex-row">
            <div
              ...
              onClick={this.showChatroomProfile}
              className="chatroomInfo-infoImg"
            >
              <img
                src={this.state.chatroom_profile_image}
                alt="chatroom profile image"
              />
            </div>
          ...
          {this.state.showChatroomProfile ? (
            <div onClick={() => this.hideChatroomProfile()} className="modal">
              <section
                ...
                className="modal-main d-flex flex-column"
              >
                <ProfileOwner
                  loadChatrooms={this.props.loadChatrooms}
                  updateJoinState={this.updateJoinState}
                  isJoined={this.state.isJoined}
                  Cid={this.state.chatroomId}
                  hideChatroomProfile={this.hideChatroomProfile}
                />
              </section>
            </div>
          ) : (
