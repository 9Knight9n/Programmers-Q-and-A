...
class SignInForm extends Component {
  ...
  async handleSubmit() {
    ...
    if (!this.emailValidation()) {
      this.setState({ email: "" });
      this.setState({
        emailCheckMassage: { massege: "Email is not valid!", active: true },
      });
      return;
    }
    ...
    if (this.state.password.length === 0) {
      ...
      return this.setState({
        passwordCheckMassage: { massage: "Enter your password", active: true },
      });
    }
    ...
    return this.setState({
      loginCheckMassage: { massage: response.data.message, active: true },
    });
  }
    ...
    return (
      <div className="Abed-css">
        ...
        <div className="signInTransfer">
          <Link
            id="GoHomepageFromSignin"
            to={sessionStorage.getItem("targetURL")}
          ></Link>
          <button name="signInButton" type="button" onClick={this.handleSubmit}>
            Sign In
          </button>
          <br />
        </div>
        ...
        <div className="validFieldsSignIn error">
          {this.state.loginCheckMassage.active
            ? this.state.loginCheckMassage.massage
            : ""}
        </div>
        <br />
        <div className="signUpTransfer">
          ...
          <Link to="/signup">
            <button name="signUpButton" type="button">
              Sign Up
            </button>
          </Link>
        </div>
