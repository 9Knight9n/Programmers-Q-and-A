...
class MyAccount extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      preview: null,
      avatarChanged: false,
      currentComponent: <React.Fragment></React.Fragment>,
    };
  ...
  async onSave() {
    ...
    const response3 = await axios.post(
      "http://404socialmedia.ir/api/editprofilepicture/",
      form,
      {
        headers: {
        ...
        <nav className="">
          <ul className="nav nav-tabs">
            ...
            {this.state.tabs.map((tab) => (
              <li
                key={tab.id}
                className="pl-1 pr-1 nav-item d-flex align-items-end"
              >
                <a
                  onClick={() => this.tabSelected(tab.id)}
                  className={"mb-0 w-100 nav-link d-flex justify-content-center"
                    .concat(this.state.selectedTab === tab.id ? " active" : "")
                    .concat(tab.id === 2 ? " disabled" : "")}
                  href="#"
                >
                  {tab.label}
                </a>
