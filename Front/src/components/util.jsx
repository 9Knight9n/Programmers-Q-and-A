...
export const decodeList = (list, selected) => {
  let result = [];
  for (let k = 0; k < list.length; k++) {
    if (selected.charAt(k) === "1") result.push(list[k]);
  }
  return result;
  // console.log(result)
};
export const getUserAvatar = async (id) => {
  ...
  // console.log(data)
  if (data.Base64) sessionStorage.setItem(id + ":avatar", data.Base64);
  else
...
export const getActiveNav = () => {
  if (window.location.pathname.includes("qanda")) return "qanda";
  if (window.location.pathname.includes("discussion")) return "discussion";
  else return "qanda";
};
export const getActiveChannel = () => {
  let temp = window.location.pathname.split(getActiveNav());
  if (temp[1]) return parseInt(temp[1]);
  else return -1;
};
