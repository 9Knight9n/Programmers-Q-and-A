...
class Search extends Component {
  ...
  handelInputChange = (event) => {
    ...
    } else this.setState({ panelOpened: false, suggestions: [] });
  };
    ...
    return (
      <React.Fragment>
        <div ref={(node) => (this.state.node = node)}>
          <Link
            id="goToSearchResultPage"
            onClick={() => this.props.changeChatroom(-1)}
            to={{
          ...
          <div id="search" className="d-flex flex-row">
            <div
              id="bar"
              className={"ml-auto d-flex flex-row-reverse".concat(
            ...
            <button
              ...
              className="ml-2 mr-4 mt-auto mb-auto rounded-circle d-flex flex-row transparent-button"
            >
              <svg
                width="27px"
                height="27px"
                viewBox="0 0 16 16"
                className=" text-white bi bi-search align-self-center"
                fill="currentColor"
            ...
            <div
              id="panel"
              className={"mt-5 mr-2 ".concat(
                this.state.panelOpened ? " active" : ""
              )}
            >
              <div
                id="search-sub-panel1"
                className={"m-3".concat(
                  this.state.panelOpened ? " " : " display-none"
                )}
              >
                {this.state.searchInput.length < 3 ? (
                  this.state.minError ? (
                    <p>Enter at least 3 letters!</p>
                  ) : (
                    ""
                  )
                ) : this.state.suggestions.length > 0 ? (
                  <p>Suggested Chatrooms :</p>
                ) : (
                  <p>can't suggest any Chatroom</p>
                )}
              </div>
              <div
                id="search-sub-panel2"
                className={"search-result".concat(
                  this.state.suggestions.length > 0 &&
