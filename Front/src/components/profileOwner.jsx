...
class ProfileOwner extends Component {
  ...
  handleSaveEdits = async (input) => {
    ...
    if (input === this.state.preview) sendElement = "chatroom_profile_image";
    let config = {
      url: "http://404socialmedia.ir/api/EditChatroomProfile/",
      needToken: false,
      type: "post",
      formKey: ["chatroomId", sendElement],
      formValue: [this.state.Cid, input],
    };
    let data = [];
    data = await request(config);
    if (data) {
  ...
  handleLeave = async () => {
    ...
    let config = {
      ...
      needToken: false,
      type: "post",
      formKey: ["chatroomId", "id"],
      formValue: [this.state.Cid, sessionStorage.getItem("id")],
    };
    let data = [];
    // console.log("outside 0",data)
    data = await request(config);
  ...
  render() {
    return (
      // <img src={profileImg} />
      <div
        className="chProfileOwner chProfileOwner-main-box"
        style={{ overflowY: "hidden" }}
      >
        <div className="chProfileOwner-exitImg">
          <svg
            src={exitImg}
            onClick={this.props.hideChatroomProfile}
          ...
          <div className="chProfileOwner-profile d-flex flex-column">
              ...
              <div className="chProfileOwner-chName-chTitle-chLink col">
                <div className="chProfileOwner-chNameBox">
                  <div className="d-flex flex-row">
                    <label className="chNameLable" for="chProfileOwner-chLink">
                      Context link :{" "}
                    </label>
                    <div className="chProfileOwner-chNameEditImg">
                      {this.state.isOwner && !this.state.OwnerIsEditingName ? (
                        <img
                        ...
                        <div className="d-flex justify-content-center">
                          <img
                            className="mr-2"
                            onClick={() => this.handleEditClick(4)}
                            alt="cancelImg"
                            data-tip="cancel"
                            alt="saveIcon"
                            src={cancelIcon}
                          ...
                          <img
                            onClick={() =>
                              this.handleSaveEdits(this.state.chatroomName)
                            }
                            data-tip="Save"
                            alt="saveIcon"
                            src={saveIcon}
                  ...
                  <div className="chProfileOwner-chName mr-3">
                    {this.state.isOwner && this.state.OwnerIsEditingName ? (
                      <input
                        ...
                        onClick={() => this.handleEditClick(7)}
                        name="chatroomName"
                        type="text"
                        value={this.state.chatroomName}
                      ></input>
                    ) : (
                      <label>{this.state.chatroomName}</label>
                    )}
                    {this.state.nameError ? (
                      <span className="error">
                        {this.state.chatroomNameMsg}
                      </span>
                    ) : (
                      ""
                    )}
                ...
                <div className="chProfileOwner-chLinkBox">
                  <div className="d-flex flex-row">
                    ...
                    <div className="chProfileOwner-chLinkEditImg">
                      {this.state.isOwner && !this.state.OwnerIsEditingLink ? (
                        <img
                          alt="editIcon"
                          data-tip="Edit"
                          src={editIcon}
                        ...
                        <div className="d-flex justify-content-center">
                          <img
                            className="mr-2"
                            onClick={() => this.handleEditClick(5)}
                            alt="cancelImg"
                            data-tip="Cancel"
                            alt="cancelIcon"
                            src={cancelIcon}
                          ...
                          <img
                            onClick={() =>
                              this.handleSaveEdits(this.state.chatroomLink)
                            }
                            data-tip="Save"
                            alt="saveIcon"
                            src={saveIcon}
                  ...
                  <div className="chProfileOwner-chLink">
                    {this.state.isOwner && this.state.OwnerIsEditingLink ? (
                      <input
                        ...
                        name="chatroomLink"
                        type="text"
                        value={this.state.chatroomLink}
                      ></input>
                    ) : (
                      <a href={this.state.chatroomLink} target="blank">
                        Click here!
                      </a>
                    )}
                    {this.state.linkError ? (
                      <span className="error">
                        {this.state.chatroomLinkMsg}
                      </span>
                    ) : (
                      ""
                    )}
            ...
            <div className="chProfileOwner-desBox">
              <div className="d-flex flex-row">
                ...
                <div className="chProfileOwner-desEditImg">
                  {this.state.isOwner && !this.state.OwnerIsEditingDes ? (
                    <img
                      alt="editIcon"
                      data-tip="Edit"
                      src={editIcon}
                    ...
                    <div className="d-flex justify-content-center">
                      <img
                        className="mr-2"
                        onClick={() => this.handleEditClick(6)}
                        alt="cancelImg"
                        data-tip="Cancel"
                        src={cancelIcon}
                      />
                      <img
                        onClick={() =>
                          this.handleSaveEdits(this.state.chatroomDes)
                        }
                        data-tip="Save"
                        alt="saveIcon"
                        src={saveIcon}
              ...
              <div className="chProfileOwner-des">
                {this.state.isOwner && this.state.OwnerIsEditingDes ? (
                  <textarea
                    ...
                    name="chatroomDes"
                    type="text"
                    value={this.state.chatroomDes}
                  ></textarea>
                ) : (
                  <p>{this.state.chatroomDes}</p>
                )}
              </div>
            </div>
