...
class GeneralChatroom extends Component {
  ...
  async componentWillMount() {
    if (this.state.ChatroomID !== -1) {
      await connect(
        "ws://404socialmedia.ir/ws/api/generalchatroom/" +
  ...
  changeJoinState = (state) => {
    ...
    } else this.setState({ inputHeight: 0 });
  };

  loadChats = async () => {
    if (this.props.match.params.chatroomid === "-1") return false;
    ...
    let config = {
      ...
      type: "post",
      formKey: ["chatroomId"],
      formValue: [this.props.match.params.chatroomid],
    };
    let data = [];
  ...
  loadData = async () => {
    ...
    let config = {
      ...
      type: "post",
      formKey: ["chatroomId"],
      formValue: [this.state.ChatroomID],
    };
    let data = [];
    data = await request(config);
    if (data) {
