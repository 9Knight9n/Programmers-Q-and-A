...
class QuestionChatbox extends Component {
  ...
  componentDidMount = async () => {
    ...
    if (this.state.isAnswered === "false") this.setState({ isAnswered: false });
  ...
  handleEdit = async () => {
    ...
    let config = {
      ...
      needToken: true,
      type: "post",
      formKey: ["chatroom", "user_id", "id", "text"],
      formValue: [
        this.props.Cid,
  ...
  handleSameProblemClicked = async (voteState) => {
    if (this.state.sameProblem === voteState) return;
    this.setState({
      sameProblem: this.state.sameProblem + voteState,
    ...
    let config = {
      ...
      needToken: true,
      type: "post",
      formKey: ["question_id", "user_id", "voteState"],
      formValue: [
        this.props.Qid,
        sessionStorage.getItem("id"),
  ...
  handleDelete = async () => {
    ...
    let config = {
      ...
      needToken: true,
      type: "post",
      formKey: ["chatroom", "user_id", "id"],
      formValue: [this.props.Cid, sessionStorage.getItem("id"), this.props.Qid],
    };
    console.log(config);
    let data = [];
    // console.log("outside 0",data)
  ...
  updateContent = (value) => {
    if (this.state.editing) this.setState({ editorContent: value });
    else this.setState({ editorContentAnswer: value });
  };
    ...
    return (
      <React.Fragment>
        <ProfilePreview
          ...
          hideProfilePreview={this.hideProfilePreview}
          showProfilePreview={this.state.showProfilePreview}
        />
        <Texteditor
        ...
        >
          <div id="header" className="d-flex flex-row ml-auto">
            ...
            <div className="d-flex pl-2 align-top w-80 ml-3" id="profile">
              <div
                ...
                onClick={this.showProfilePreview}
              >
                <img id="profile-img" src={this.state.senderAvatar} />
              </div>
              <p
                style={{ cursor: "pointer" }}
                onClick={this.showProfilePreview}
                className="pt-1 h5 d-flex align-items-center pr-4 "
              >
            ...
            <div id="options" className="ml-auto">
              <Dropdown>
                ...
                <Dropdown.Menu className="dropDown">
                  ...
                  <hr />
                  {this.state.isOwner ? (
                    <Dropdown.Item as="button" onClick={this.startEditing}>
                      Edit
                    </Dropdown.Item>
                  ) : (
                    ""
                  ...
                  <hr />
                  {this.state.isOwner ? (
                    <Dropdown.Item as="button" onClick={this.handleDelete}>
                      Delete
                    </Dropdown.Item>
                  ) : (
                    ""
          ...
          <div id="body" className="d-flex flex-row w-100">
            <div id="left" className="d-flex flex-column mt-2">
              <button
                style={{ outline: "none" }}
                className="ml-auto mr-auto pr-2 pl-2 mb-2 mt-2 clean-button"
              ...
              <button
                style={{ outline: "none" }}
                class="ml-auto mr-auto pr-2 pl-2 mt-1 clean-button"
                data-tip="Number of users with same problem!"
              >
              ...
              {this.state.isAnswered ? (
                <svg
                  style={{ fill: "green" }}
                  data-tip="This Question is answered"
                  width="1em"
                  height="1em"
                  viewBox="0 0 16 16"
            ...
            <div id="middle" className="mt-2 mb-1 d-flex flex-column">
              ...
              <small className="date ml-auto mb-2 mr-2 mt-auto">
                Submitted on : {this.state.sentDate}
              </small>
            </div>
          ...
          <div id="footer" className="d-flex flex-row">
            <div className="ml-auto mr-2 mb-auto mt-auto parisa-css">
              {this.state.showMoreButton ? (
                <button
                  onClick={this.goToAnswerPage}
                  style={{ outline: "none", borderRadius: "5px" }}
                  className="Question-showAnswerButton pr-2 pl-2 m-1 btn-sm btn btn-primary"
                >
                  Show answers
                </button>
              ) : (
                <button
                  ...
                  className="pr-2 pl-2 m-1 btn-sm btn btn-primary"
                >
