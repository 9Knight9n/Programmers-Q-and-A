...
class ProfileOne extends Component {
  ...
  async loadData() {
    ...
    const response = await axios.post(
      "http://404socialmedia.ir/api/showpersonalinfo/",
      form,
      {
        headers: {
  ...
  async handleSubmit() {
    this.setState({ loading: true });
    if (
      this.state.username.startsWith("user-") &&
      this.state.username !== "user-" + sessionStorage.getItem("id")
    )
    ...
    const response = await axios.post(
      "http://404socialmedia.ir/api/getpersonalinfo/",
      form,
      {
        headers: {
          "Content-Type": "multipart/form-data",
        ...
        <div class="h-100 parisa-css content-form1 d-flex justify-content-center align-items-center">
          <div className="INPUT-FORM1">
            ...
            {this.state.usernameError ? (
              <p className="pro-error">Username is not available</p>
            ) : (
              <br />
            )}
            <p>Email :</p>
            ...
            <button
              className="profile-savebutton btn btn-primary"
              onClick={this.handleSubmit}
            >
              Save
            </button>
            {this.state.succeed ? (
              <p className="pro-success">Saved successfully!</p>
            ) : (
              <br />
            )}
          </div>
