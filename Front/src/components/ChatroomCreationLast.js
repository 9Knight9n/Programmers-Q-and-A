...
class ChatroomCreationLast extends Component {
  ...
  async handleClick() {
    ...
    if (this.state.avatarChanged) form.set("Base64", this.state.src);
    ...
    const response = await axios.post(
      "http://404socialmedia.ir/api/createchatroom/",
      form,
      {
        headers: {
    ...
    return (
      <div className="main-box">
        <div className="elements">
          ...
          {this.state.error ? (
            <div className="LastError">Please select a name for chatroom</div>
          ) : (
            ""
          )}
          <Link to={this.handleBack}>
            <button className="backButtonLast" type="button">
              Back
            </button>
          </Link>
          <Link
            to={this.state.chatroomName ? "/" : "/chatroomCreationLast"}
            onClick={this.handleClick}
          >
            <button className="createButtonLast" type="button">
              Create
            </button>
          </Link>
        </div>
