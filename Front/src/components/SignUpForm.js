...
class SignUpForm extends Component {
  ...
  handleChange(e) {
    ...
    if (
      ...
      this.setState({ error: false });
      this.setState({
        passwordCheckMassage: {
          massage:
            "Password must contains Uppercase, Lowercase, digit and at least 8 characters",
          active: true,
        },
      });
    } else {
    ...
    if (name === "confirmPassword") {
      ...
      this.setState({
        ConfirmPasswordCheckMassage: { massage: "", active: true },
      });
    }
  }
  ...
  async handleSubmit() {
    ...
    if (!this.emailValidation()) {
      ...
      return this.setState({
        emailCheckMassage: { massage: "Email is not valid!", active: true },
      });
    }
    ...
    if (response.data.message === "New user created") {
      ...
      const response3 = await axios.post(
        "http://404socialmedia.ir/api/show_profile_picture/",
        form,
        {
          headers: {
  ...
  checkPassword() {
    ...
    // If confirm password not entered
      ...
      this.setState({
        ConfirmPasswordCheckMassage: {
          massage: "Please enter confirm password",
          active: true,
        },
      });
    ...
    // If Not same return False.
      ...
      this.setState({
        ConfirmPasswordCheckMassage: {
          massage: "Password did not match: Please try again...",
          active: true,
        },
      });
      return false;
    ...
    return (
      <div className="Abed-css">
        ...
        <div className="validEmadilSignUp error">
          {this.state.emailCheckMassage.active
            ? this.state.emailCheckMassage.massage
            : ""}
        </div>
        ...
        <div
          className={"passValidSignUp".concat(this.state.error ? " error" : "")}
        >
          {this.state.passwordCheckMassage.active
            ? this.state.passwordCheckMassage.massage
            : ""}
        </div>
        ...
        <div
          className={"confirmPassConflict".concat(
            this.state.error ? " error" : ""
          )}
        >
          {this.state.ConfirmPasswordCheckMassage.active
            ? this.state.ConfirmPasswordCheckMassage.massage
            : ""}
        </div>

        <div className="signUpTransfer2">
          <Link
            id="GoHomepageFromSignup"
            to={sessionStorage.getItem("targetURL")}
          ></Link>
          <button
            name="signUpButton2"
            type="button"
        ...
        <div className="validFieldSignUp error">
          {this.state.signUpCheckMassage.active
            ? this.state.signUpCheckMassage.massage
            : ""}
        </div>
        <br />
        <div className="signInTransfer2">
          ...
          <Link to="/login">
            <button name="signInButton2" type="button">
              Sign In
            </button>
          </Link>
        </div>
